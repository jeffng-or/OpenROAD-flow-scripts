...
"""
parser.add_argument(
    "--verbose", "-v", required=False, default=0, action="count", help=HELP_TEXT
)
...
def gen_report(name, data):
    ...
    output = append_text(
        data["log_errors"], output, "errors in the logs", REGEX_ERROR, args.verbose
    )
    output = append_text(
    ...
    output = append_text(
        data["metrics_errors"], output, "metrics failures", REGEX_ERROR, args.verbose
    )
    output = append_text(
        data["calibre_errors"], output, "calibre failures", REGEX_ERROR, args.verbose
    )
...
for log_dir, dirs, files in sorted(os.walk(LOGS_FOLDER, topdown=False)):
    ...
    d["calibre_errors"], d["calibre_warnings"] = parse_messages(
        os.path.join(log_dir, "calibre/save-to-drc-db.log"), print_missing=False
    )
